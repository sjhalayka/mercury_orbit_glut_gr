...
custom_math::vector_3 grav_acceleration(const custom_math::vector_3& pos, const custom_math::vector_3& vel, const double G)
{
	...
	const double L = sqrt(pos.length() * vel.length());

	double a = (G * sun_mass) / (distance * distance)
		- (L * L) / (distance * distance * distance)
		- (3.0 * G * sun_mass * L * L) / (speed_of_light * speed_of_light * distance * distance * distance * distance);
